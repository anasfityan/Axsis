...
const copy: Record<string, Translation> = {
  ...
  'تعمل البيانات محليًا أولًا، ثم تُرفع وتُنزل تدريجيًا عبر Firestore. يبقى Google Drive مخصصًا للملفات والنسخ الاحتياطية الاختيارية، ولا يمنع فشل أي خدمة سحابية استخدام التطبيق محليًا.': { en: 'Data works locally first, then syncs gradually through Firestore. Google Drive remains optional for files and backups, and cloud failures never block local use.', tr: 'Veriler önce yerel çalışır, ardından Firestore üzerinden kademeli eşitlenir. Google Drive dosya ve yedekler için isteğe bağlıdır; bulut hataları yerel kullanımı engellemez.' },
  ...
  'يعمل دون إنترنت': { en: 'Works offline', tr: 'Çevrimdışı çalışır' },
...
let originalAlert: typeof window.alert | null = null
...
function installDialogs(locale: Locale): () => void {
  ...
  window.confirm = (message?: string) => originalConfirm!(translate(String(message ?? ''), currentLocale))
  window.alert = (message?: string) => originalAlert!(translate(String(message ?? ''), currentLocale))
  return () => {
